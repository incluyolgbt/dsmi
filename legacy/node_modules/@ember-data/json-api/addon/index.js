...
class JSONAPICache {
   ...
   */
  put(doc) {
    assert(`Expected a JSON:API Document as the content provided to the cache, received ${doc.content}`, doc instanceof Error || typeof doc.content === 'object' && doc.content !== null);
  ...
  clientDidCreate(identifier, options) {
    ...
    if (options !== undefined) {
      ...
      for (let i = 0; i < propertyNames.length; i++) {
        ...
        const fieldType = relationshipDefs[name] || attributeDefs[name];
            ...
            Object.keys(relationships).forEach(relationshipName => {
              ...
              if (relationship.kind === 'belongsTo') {
                ...
                if (relationshipData !== undefined) {
                  ...
                  const actualData = relationshipData ? this.__storeWrapper.identifierCache.getOrCreateRecordIdentifier(relationshipData) : null;
                  assert(`Expected the resource relationship '<${identifier.type}>.${relationshipName}' on ${identifier.lid} to be saved as ${inFlightData.data ? inFlightData.data.lid : '<null>'} but it was saved as ${actualData ? actualData.lid : '<null>'}`, inFlightData.data === actualData);
                }
              }
            });
            // @ts-expect-error debugging only property
...
function putOne(cache, identifiers, resource) {
  ...
  assert(`Missing Resource Type: received resource data with a type '${resource.type}' but no schema could be found with that name.`, cache.__storeWrapper.getSchemaDefinitionService().doesTypeExist(resource.type));
...
function _directlyRelatedIdentifiersIterable(storeWrapper, originating) {
  ...
  const findNext = () => {
    while (i < initializedRelationshipsArr.length) {
      while (j < 2) {
        let relatedIdentifiers = j === 0 ? getLocalState(initializedRelationshipsArr[i]) : getRemoteState(initializedRelationshipsArr[i]);
